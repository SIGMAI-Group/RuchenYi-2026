...
topk_metrics = {metric.lower(): metric for metric in ['Recall', 'Recall2', 'Precision', 'NDCG', 'MAP','Target_HR', 'Target_NDCG']}
...
class TopKEvaluator(object):
    ...
    #--------------------------------------attack metrics
        if 'target_items' in self.config:
            target_items = self.config['target_items']
            
            # 1. 构造 Target 命中矩阵
            target_bool_matrix = np.isin(topk_index, target_items)
            
            # 2. 构造 Target 长度列表
            target_len_list = np.full(len(topk_index), len(target_items))
            
            # 3. 指定要计算的攻击指标，调用字典里的函数进行运算
            attack_metrics = ['target_hr', 'target_ndcg']
            for metric in attack_metrics:
                metric_fuc = metrics_dict[metric]
                value = metric_fuc(target_bool_matrix, target_len_list)
                for k in self.topk:
                    key = '{}@{}'.format(topk_metrics[metric], k)
                    # 核心：直接往系统原有的 metric_dict 里追加新的键值对
                    metric_dict[key] = round(value[k - 1], 4)
    #--------------------------------------attack metrics
        return metric_dict
